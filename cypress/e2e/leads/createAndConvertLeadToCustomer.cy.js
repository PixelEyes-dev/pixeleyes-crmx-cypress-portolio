...
describe("Generate Leads", () => {
    ...
    cy.get("tr td:nth-child(4)")
      .each(($el, index, $list) => {
        ...
        if (emailText === createdLeadEmail) {
          ...
          LeadsPage.convertLeadToCustomerTitle()
            .should("be.visible")
            .should("contain", "Convert to Customer");

          // Wait for the dialog content to be fully rendered
          cy.get("#convertLeadDialogContent").should("be.visible");

          // Wait for the form to be fully loaded and user data to be available
          cy.get("#customer-tab-basic").should("be.visible");

          // Wait for the form fields to be populated (indicating user data is loaded)
          cy.get("#customer-input-first-name").should("be.visible");

          // Wait for the save button to be enabled and ready
          cy.get("#customer-btn-save")
            .should("be.visible")
            .and("not.be.disabled");
          ...
          cy.get("#customer-btn-save").should("be.enabled");

          // Wait for the form to be fully ready by checking multiple elements
          cy.get("#customer-input-first-name").should("be.visible");
          cy.get("#customer-btn-save").should("be.visible").and("be.enabled");

          // Remove debug and pause
          // cy.debug();
          // cy.pause();
  ...
  it("should verify the converted lead is now a customer in the FE", () => {
    ...
    CustomersPage.customersPageTitle()
      .should("be.visible")
    ...
    cy.get("tr td:nth-child(4)")
      .each(($el, index, $list) => {
        ...
        if (emailText === createdLeadEmail) {
          emailFound = true;
          cy.log(
            `✅ Found converted lead to customer email in row ${
              index + 1
            }: ${emailText}`
          );
          // break out of .each()
      ...
      .then(() => {
        if (!emailFound) {
          cy.log(
            `❌ Converted lead to customer lead email not found in table: ${createdLeadEmail}`
          );
          throw new Error(
            `Converted lead with email "${createdLeadEmail}" was not found in the customers table`
          );
        }
      });
  });
    ...
    cy.get("tr td:nth-child(4)")
      .each(($el, index, $list) => {
        ...
        if (emailText === createdLeadEmail) {
          ...
          CustomersPage.successConfirmationMessage().should(
            "contain",
      ...
      .then(() => {
        if (!emailFound) {
          ...
          throw new Error(
            `Customer with email "${createdLeadEmail}" was not found in the customers table`
          );
        }
      });
