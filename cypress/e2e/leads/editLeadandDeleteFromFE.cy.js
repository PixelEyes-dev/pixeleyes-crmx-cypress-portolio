    ...
    cy.get('tr td:nth-child(4)')
      .each(($el, index, $list) => {
        ...
        if (emailText === createdLeadEmail) {
          ...
          LeadsPage.leadEditTitle().should('be.visible').should('contain', 'Edit Lead');
          LeadsPage.addNewLeadEmailTextBox().clear();
          LeadsPage.addNewLeadEmailTextBox().type(editedLeadEmail);
          LeadsPage.addNewLeadSaveButton().click();
          LeadsPage.successConfirmationMessage().should('contain', 'Lead updated successfully');
          // break out of .each()
          return false;
        }
      })
      .then(() => {
        if (!emailFound) {
          ...
          throw new Error(`Lead with email "${editedLeadEmail}" was not found in the leads table`);
        }
      });
  });
    ...
    cy.get('tr td:nth-child(4)')
      .each(($el, index, $list) => {
        ...
        if (emailText === editedLeadEmail) {
          emailFound = true;
          cy.log(`✅ Found edited lead email in row ${index + 1}: ${emailText}`);
          // break out of .each()
          return false;
        }
      })
