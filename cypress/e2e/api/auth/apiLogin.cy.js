describe('Authentication API Tests', () => {
  ...
  it('should successfully authenticate with valid credentials', () => {
    ...
    cy.request({
      ...
      if (responseBody.user.created_at) {
        expect(responseBody.user.created_at).to.be.a('string');
        expect(new Date(responseBody.user.created_at)).to.be.instanceOf(Date);
      }

      if (responseBody.user.updated_at) {
        expect(responseBody.user.updated_at).to.be.a('string');
        expect(new Date(responseBody.user.updated_at)).to.be.instanceOf(Date);
      }
  ...
  it('should verify user session after successful login', () => {
    cy.request({
      method: 'POST',
      url: `${SUPABASE_URL}/auth/v1/token?grant_type=password`,
      headers: {
      ...
      cy.request({
        ...
        if (sessionBody.user) {
          ...
          if (sessionBody.user.updated_at) {
            expect(sessionBody.user.updated_at).to.be.a('string');
            expect(new Date(sessionBody.user.updated_at)).to.be.instanceOf(Date);
          }
        } else {
